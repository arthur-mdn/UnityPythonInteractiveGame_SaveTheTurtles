...
# Charger le modèle YOLO pour la détection de pose
pose_model = YOLO('yolov8n-pose.pt')

def detect_and_draw_poses(frame):
    # Effectuer la détection de pose
    results = pose_model(frame)
    
    # Dessiner les poses sur le frame
    frame_with_poses = results.render()[0]
...
def main():
    ...
    while True:
        ...
        # Appliquer la détection de pose
        frame_with_poses = detect_and_draw_poses(frame)

        # Notre logique de traitement d'opérations sur le frame vient ici
        processed_frame, player_position = detect_and_draw_player(frame_with_poses, backSub)

        # Détecter les marqueurs ArUco et obtenir la position transformée si les marqueurs sont présents
        marker_positions = detect_aruco_markers(processed_frame)
        if marker_positions:
            # Assurez-vous que tous les marqueurs nécessaires sont présents
            if all(marker in marker_positions for marker in [10, 80]):
                # Obtenir les points de transformation et transformer la position du joueur
                points = np.array([marker_positions[10], marker_positions[80]], dtype="float32")
                transformed_player_position = four_point_transform(frame, points)
                if player_position is not None:
                    # Transformer la position du joueur
                    player_position = four_point_transform(np.array([player_position], dtype="float32"), points)
                    cv2.circle(transformed_player_position, (int(player_position[0][0]), int(player_position[0][1])), 10, (0, 0, 255), -1)  # Dessine un cercle rouge
                # Afficher la position transformée sur le cadre traité pour une vérification visuelle
                cv2.imshow('Transformed Frame', transformed_player_position)
