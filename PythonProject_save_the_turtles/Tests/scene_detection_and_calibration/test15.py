...
def process_frame(frame):
    ...
    # Appliquez votre logique de traitement d'image ici
    # Par exemple, auto_detect_edges mais sur un frame plutôt qu'une image lue depuis un fichier
    orig = frame.copy()
    ...
    if screenCnt is not None:
        points = screenCnt.reshape(4, 2)
        ordered_points = order_points_with_aruco_markers(points, marker_positions)
        warped = four_point_transform(orig, ordered_points)
        return warped
    else:
        print("No suitable contour found")
        return frame
